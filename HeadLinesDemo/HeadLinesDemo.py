...
while True:
  #  _, frame = cap.read()
    nose_mask.fill(0)
    gray_frame = cv2.cvtColor(test, cv2.COLOR_BGR2GRAY)
    faces = detector(test)
    ...
    for face in faces:
       ...
       # for n in range(17, 26):
        ...
        cv2.circle(test, leftSide, 4, (255, 0, 0), -1)

        rightSide= (landmarks.part(26).x, landmarks.part(26).y)
        cv2.circle(test, rightSide, 4, (255, 0, 0), -1)
       ...
       # cv2.line(frame, leftSide, rightSide, (255, 0, 0))
        ...
        cv2.rectangle(test,leftSide, rightSide, (255, 0, 0))
